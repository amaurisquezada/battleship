...
const regularAI = (settings = { normalVariation: true, normalAttack: true, shipReveal: true }) => {
  ...
  const priorityOrder = (ships) => {
    const shipToSearch = _(ships)
        .pickBy((v, k) => _.some(v, num => num != k && num != 0))
        .map((v, k, i) => {
          let minStrikes = _.min(_.pull(_.flatten(_.values(i)), 0))
          if (_.some(v, n => n === minStrikes)) return k})
        .value()
    return _.toInteger(_.max(shipToSearch))
  }

  const allZeros = (board, shipSize = 1) => {
    let res = []
    if (shipSize === 1) {
      res = board.reduce((final, column, y) => {
      ...
      res = board.reduce((final, column, y) => {
        column.forEach((row, x) => {
          if (row === 0 && 
              horizontalSpace(y, x, board) >= shipSize - 1 && 
              verticalSpace(y, x, board) >= shipSize - 1) {
            final.push([y, x])
          }
        })
        return final
  ...
  const horizontalSpace  = (y, x, board) => {
    return spaceToLeft(y, x, board) + spaceToRight(y, x, board)
  }

  const verticalSpace = (y, x, board) => {
    return spaceAboveCell(y, x, board) + spaceBelowCell(y, x, board)
  }
  ...
  return {
    ...
    makeGuess: (targetingBoard, ships = { 5:[5], 4:[4], 3:[3], 2:[2], 1:[1] }) => {
      ...
      if (!shipToSearch) {
        const shipToSearch = _.max(_.map(_.keys(ships), k => _.toInteger(k)))
        const guess = _.shuffle(allZeros(board, shipToSearch)).pop()
...
//         ships["2"][1] ? ships["2"][1]-- : ships["2"][0]--
//         if (ships["2"] && ships["2"][1] === 0) {
//           wb = wb.map((column, y) => {
//             return (
//               column.map((row, x) => {
//                 if (row === 2){
